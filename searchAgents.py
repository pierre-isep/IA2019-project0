...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        """
        Let's store the walls, pacman's starting position and corners.
        """
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # On met la position de départ et les nodes qui ont été visitées dans une liste 
...
def cornersHeuristic(state, problem):
    ...
    # Let's store our unvisited corners
    for corner in corners:
        if not corner in visitedCorners:
            unvisitedCorners.append(corner)


    # the idea here is that we compute the sum of the distance between our position and the closest corner, then we sum it up 
    # with the distance between this corner and the next closest corner

    while len(unvisitedCorners) > 0:
        mini = []
        for corner in unvisitedCorners:
            distance = util.manhattanDistance(node, corner)
            mini.append((distance,corner))
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        actionList = search.astar(problem)
